...
export class SchematicManager {
    constructor(schematicRenderer, options = {}) {
        Object.defineProperty(this, "schematics", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        ...
        Object.defineProperty(this, "singleSchematicMode", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    readFileWithProgress(file, onProgress) {
    ...
    async loadSchematic(name, schematicData, properties, options) {
        ...
        if (schematicData instanceof ArrayBuffer) {
            ...
            schematicWrapper = schematicData;
        }
        else {
            throw new Error(`Invalid schematic data type. Expected ArrayBuffer or SchematicWrapper. Found: ${typeof schematicData}. Object: ${JSON.stringify(Object.getOwnPropertyNames(schematicData))}`);
        }
        options?.onProgress?.({
            stage: "parsing",
            progress: 20,
        ...
        const schematicObject = new SchematicObject(this.schematicRenderer, name, schematicWrapper, properties);
        options?.onProgress?.({
            stage: "mesh_building",
            progress: 60,
    ...
    async loadSchematics(schematicDataMap, propertiesMap) {
    ...
    async loadSchematicFromFile(file, options) {
        try {
            ...
            const arrayBuffer = await this.readFileWithProgress(file, (progress) => {
                ...
                if (this.schematicRenderer.options.enableProgressBar && this.schematicRenderer.uiManager) {
                    this.schematicRenderer.uiManager.updateProgress(progress / 100, // Convert to 0-1 range
                    `Reading ${file.name}...`);
                }
            });
    ...
    async loadSchematicFromURL(url, name, properties, options) {
        try {
    ...
    getSchematicAtPosition(position) {
     ...
     */
    getMaxSchematicTightDimensions() {
        if (this.isEmpty())
            return new THREE.Vector3();
        const maxDimensions = new THREE.Vector3();
        const schematics = this.getAllSchematics();
        for (const schematic of schematics) {
            ...
            if (tightDimensions[0] === 0 && tightDimensions[1] === 0 && tightDimensions[2] === 0) {
                ...
                maxDimensions.max(new THREE.Vector3(tightDimensions[0], tightDimensions[1], tightDimensions[2]));
            }
        }
        return maxDimensions;
    }
    ...
    createEmptySchematic(name, options) {
        const schematicWrapper = new SchematicWrapper();
        const schematicObject = new SchematicObject(this.schematicRenderer, name, schematicWrapper, options);
        this.addSchematic(schematicObject);
