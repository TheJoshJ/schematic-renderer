...
export class CameraManager extends EventEmitter {
    constructor(schematicRenderer, options = {}) {
        ...
        Object.defineProperty(this, "cameraOptions", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "activeCameraKey", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "activeControlKey", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "animationRequestId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "animationStartPosition", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new THREE.Vector3()
        });
        Object.defineProperty(this, "animationStartRotation", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new THREE.Euler()
        });
        Object.defineProperty(this, "recordingManager", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "autoOrbitEnabled", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        });
        Object.defineProperty(this, "autoOrbitAnimationId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "autoOrbitStartTime", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "autoOrbitDuration", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 30
        }); // seconds for full 360° rotation
        Object.defineProperty(this, "cameraPathManager", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        const defaultPreset = CameraManager.CAMERA_PRESETS[defaultPresetName] || CameraManager.CAMERA_PRESETS.perspective;
        ...
        const initialPreset = CameraManager.CAMERA_PRESETS[this.activeCameraKey];
        ...
        if (initialPreset.controlType === "orbit" && this.schematicRenderer.keyboardControls) {
            ...
            if (initialControls) {
                this.schematicRenderer.keyboardControls.setOrbitControls(initialControls);
            }
        }
    ...
    createCamera(type, params) {
        let camera;
        if (type === "perspective") {
            camera = new CameraWrapper("perspective", this.rendererDomElement, this.schematicRenderer, params);
        }
        else {
            camera = new CameraWrapper("orthographic", this.rendererDomElement, this.schematicRenderer, params);
        }
        return camera;
    }
    getDefaultCameraPath() {
    ...
    async animateCameraAlongPath(pathOrOptions) {
        let cameraPath;
        let options = {};
        ...
        const { totalFrames = 300, targetFps = 60, easing = EasingFunctions.linear, lookAtTarget = true, updateControls = true, onStart, onUpdate, onComplete, } = options;
        ...
        try {
            return new Promise((resolve, _) => {
                ...
                const { position: startPosition, rotation: startRotation, target, } = cameraPath.getPoint(0);
                this.activeCamera.position.copy(startPosition);
                this.activeCamera.rotation.copy(startRotation);
                if (lookAtTarget) {
    ...
    switchCameraPreset(presetName) {
        const preset = CameraManager.CAMERA_PRESETS[presetName];
        if (!preset) {
        ...
        if (activeControl) {
            ...
            if (preset.controlType === "orbit" && activeControl.update) {
                ...
                if (this.schematicRenderer.keyboardControls) {
                    this.schematicRenderer.keyboardControls.setOrbitControls(activeControl);
                }
            }
            else {
    ...
    setupIsometricControls(controls) {
     ...
     */
    async focusOnSchematics(options = {}) {
        ...
        const { padding = 0.05, // 5% padding - tight but with small safety margin
        animationDuration = 0, easing = (t) => t * t * (3.0 - 2.0 * t), // smooth step
        skipPathFitting = false, } = options;
        ...
        const startPosition = new THREE.Vector3().copy(this.activeCamera.position);
        const startRotation = new THREE.Euler().copy(this.activeCamera.rotation);
        let targetPosition;
        ...
        if (this.activeCamera.camera.type === "OrthographicCamera") {
            ...
            const orthoCamera = this.activeCamera.camera;
        ...
        if (animationDuration > 0) {
            await this.animateToPosition(startPosition, startRotation, targetPosition, targetRotation, lookAtTarget, animationDuration, easing);
        }
        else {
        ...
        else if (this.schematicRenderer.options.enableAutoOrbit &&
            animationDuration === 0 &&
     ...
     */
    calculateOrthographicSize(objectSize, aspect, padding) {
     ...
     */
    async startAutoOrbitWithZoomIn(options = {}) {
        const { zoomIn = true, zoomDuration = 2.0, orbitDelay = 0.5, orbitTransitionDuration = 1.0, } = options;
     ...
     */
    setZoomInOnLoad(enabled, duration = 2.0) {
     ...
     */
    async handleSchematicLoaded(enableZoomIn = false) {
        if (enableZoomIn) {
     ...
     */
    startAutoOrbitFromOptimalPosition() {
        ...
        const circularPath = defaultPath.path;
        // Find a good starting point on the orbit (current camera position projected onto orbit)
        const currentPosition = new THREE.Vector3().copy(this.activeCamera.position);
    ...
    startAutoOrbitSmooth(options = {}) {
        ...
        const { transitionDuration = 1.0, startFromCurrentPosition = true, easing = (t) => t * t * (3.0 - 2.0 * t), skipPathFitting = false, } = options;
     ...
     */
    async zoomInToSchematics(options = {}) {
        const { padding = 0.08, duration = 2.0, easing = (t) => t * t * (3.0 - 2.0 * t), startDistance = 3.0, startFromCurrentPosition = false, skipPathFitting = false, // Don't update path during zoom sequence
         } = options;
        if (this.schematicRenderer?.schematicManager?.isEmpty())
            return;
        ...
        let finalTargetRotation = null;
        let finalLookAtTarget = center;
        if (this.activeCamera.camera.type === "OrthographicCamera") {
            const result = this.calculateIsometricFraming(center, size, aspect, padding);
        ...
        let zoomStartPosition;
        const currentCamRotation = this.activeCamera.rotation
            ? this.activeCamera.rotation.clone()
            : new THREE.Euler();
        if (startFromCurrentPosition) {
            zoomStartPosition = this.activeCamera.position.clone();
        }
        else {
        ...
        if (controls)
            controls.enabled = false;
        await this.animateToPosition(zoomStartPosition, currentCamRotation, // Current rotation
        finalTargetPosition, finalTargetRotation, // Target rotation (for ortho)
     ...
     */
    async transitionToOrbit(circularPath, duration, easing) {
        const startPosition = this.activeCamera.position.clone();
        const startRotation = this.activeCamera.rotation.clone();
        ...
        await this.animateToPosition(startPosition, startRotation, // Current rotation
        targetPositionOnOrbit, null, // Rotation will be handled by lookAt for perspective, ortho maintains its rotation
     ...
     */
    findClosestOrbitPoint(circularPath, position) {
        ...
        if (dirToProjected.lengthSq() === 0) {
            ...
            return {
                position: new THREE.Vector3(pathCenter.x + radius * Math.cos(angle), pathCenter.y + height, pathCenter.z + radius * Math.sin(angle)),
                angle: angle,
            };
        }
        dirToProjected.normalize();
        const angle = Math.atan2(dirToProjected.z, dirToProjected.x);
        const orbitPosition = new THREE.Vector3(pathCenter.x + radius * Math.cos(angle), pathCenter.y + height, pathCenter.z + radius * Math.sin(angle));
        return { position: orbitPosition, angle };
    }
    /**
     * Animate camera to target position smoothly
     */
    async animateToPosition(startPos, startRotEuler, targetPos, targetRotEuler, // Target Euler for ortho, null for perspective (uses lookAt)
    lookAtTarget, // Point to look at, primarily for perspective
    duration, easing) {
        return new Promise((resolve) => {
            const startTime = performance.now();
            const startRotQuat = new THREE.Quaternion().setFromEuler(startRotEuler);
            let targetRotQuat = null;
            if (targetRotEuler) {
            ...
            const animate = () => {
                ...
                if (this.activeCamera.camera.type === "OrthographicCamera" && targetRotQuat) {
                    ...
                    const eulerRot = new THREE.Euler().setFromQuaternion(currentRot, this.activeCamera.camera.rotation.order);
    ...
    calculateSchematicBounds() {
        ...
        console.log(`[CameraManager] World-space bounds: center=${center.toArray()}, size=${size.toArray()}`);
     ...
     */
    calculateOptimalViewingAngles(objectSize, screenAspect = 1.0) {
     ...
     */
    calculatePerspectiveFraming(center, objectSize, // Full object dimensions (x, y, z)
    aspect, padding) {
        ...
        const rotatedCorners = corners.map((c) => new THREE.Vector3(c.dot(camRight), c.dot(camUp), c.dot(camBack)));
        ...
        for (let i = 0; i < 3; i++) {
            ...
            for (const p of rotatedCorners) {
                const dist = d - p.z;
                if (dist < 0.001)
                    continue; // Should not happen with min distance
                const u = (p.x - cx) / dist;
                const v = (p.y - cy) / dist;
            ...
            cy += centerV * d;
            // Stop if converged
            if (Math.abs(centerU) < 0.001 && Math.abs(centerV) < 0.001)
                break;
        }
     ...
     */
    calculateIsometricFraming(center, objectSize, 
     ...
     */
    async zoomToOrbitPosition(options = {}) {
        const { duration = 2.0, 
        // @ts-ignore
        padding = 0.08, startFromCurrentPosition = true, startOrbitAfterZoom = false, easing = (t) => t * t * (3.0 - 2.0 * t), } = options;
        if (this.schematicRenderer?.schematicManager?.isEmpty()) {
            return;
        }
        ...
        if (!defaultPath || !(defaultPath.path instanceof CircularCameraPath)) {
            ...
            if (startOrbitAfterZoom)
                this.startAutoOrbitFromOptimalPosition(); // Fallback to just orbit
            return;
        }
        const circularPath = defaultPath.path;
        ...
        let zoomStartPosition;
        const currentCamRotation = this.activeCamera.rotation.clone();
        ...
        const idealOrbitPoint = this.findClosestOrbitPoint(circularPath, startFromCurrentPosition
            ? this.activeCamera.position.clone()
        ...
        await this.animateToPosition(zoomStartPosition, currentCamRotation, idealOrbitPoint.position, // Target position on orbit
...
Object.defineProperty(CameraManager, "CAMERA_PRESETS", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: {
        isometric: {
            type: "orthographic",
            position: [0, 0, 20], // Initial position
        ...
        perspective: {
            type: "perspective",
            position: [0, 20, 20],
            rotation: [(-20 * Math.PI) / 180, 0, 0], // 20° down tilt
            controlType: "orbit",
