...
export class Grid extends THREE.Object3D {
    // @ts-ignore
    getLine(start, end, color = 0xffffff, lineWidth = 1, resolution = new THREE.Vector2(window.innerWidth, window.innerHeight)) {
        // Create the geometry
    ...
    constructor(
    // @ts-ignore
    camera, gridSize = 1000, majorStep = 8, 
    // @ts-ignore
    minorStep = 1, majorColor = 0xaaaaaa, 
    // @ts-ignore
    minorColor = 0x666666, fadeDistance = 100 // Distance at which grid starts fading
    ) {
        super();
        // @ts-ignore
        Object.defineProperty(this, "majorGrid", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        // @ts-ignore
        Object.defineProperty(this, "minorGrid", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "majorStep", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    createGridLines(camera, step, color, 
    // @ts-ignore
    lineWidth, 
    // @ts-ignore
    ignoreZero = false) {
    ...
    // @ts-ignore
    getGridLines(step, color, lineWidth, ignoreZero = false) {
        const lines = [];
        for (let i = 0; i <= this.gridSize; i += step) {
            if (ignoreZero && i === 0)
                continue;
    ...
    addLabels() {
        ...
        for (let i = 0; i <= this.fadeDistance; i += step) {
            for (let j = 0; j <= this.fadeDistance; j += step) {
                ...
                this.addLabel(i.toString() + "," + j.toString(), new THREE.Vector3(i, 0, j), // Directly on the grid
                labelColor, false // Not an axis label
                );
                this.addLabel(-i.toString() + "," + j.toString(), new THREE.Vector3(-i, 0, j), // Directly on the grid
                labelColor, false // Not an axis label
                );
     ...
     */
    addLabel(text, position, color, isAxisLabel) {
