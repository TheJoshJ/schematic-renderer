...
export class ResourcePackManager {
    constructor(options = {}) {
        ...
        // Batch mode
        Object.defineProperty(this, "batchMode", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        // Auto-rebuild
        Object.defineProperty(this, "autoRebuild", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: true
        ...
        Object.defineProperty(this, "downloadCache", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        ...
        Object.defineProperty(this, "onAtlasRebuild", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
     ...
     */
    setAtlasRebuildCallback(callback) {
     ...
     */
    async loadPackFromBlob(blob, name, options = {}) {
        const packId = generatePackId();
     ...
     */
    async uploadPack(file) {
        return await this.loadPackFromFile(file);
    }
    // ==================== Pack Parsing ====================
    async parsePackBlob(blob, overrideName) {
     ...
     * Legacy method for backward compatibility
     */
    async togglePackEnabled(name, enabled) {
     ...
     * Legacy method for backward compatibility
     */
    async reorderPack(name, newOrder) {
        const pack = this.getPackByName(name);
        if (!pack) {
            throw new Error(`Pack ${name} not found`);
        }
    ...
    async previewPackTexture(packId, texturePath) {
        ...
        if (!pack)
            return null;
        const zip = await JSZip.loadAsync(pack.data);
        const file = zip.file(`assets/minecraft/textures/${texturePath}.png`) ||
    ...
    async validatePack(blob) {
        ...
        try {
            ...
            errors.push(`Failed to read zip file: ${error instanceof Error ? error.message : String(error)}`);
        }
        return {
     ...
     */
    async getResourcePackBlobs(defaultPacks = {}) {
        await this.ensureDbInitialized();
