 ...
 */
export class InsignIoHoverHandler {
    constructor(renderer) {
        ...
        Object.defineProperty(this, "currentHoveredRegion", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "onMouseMove", {
            ...
            writable: true,
            value: (event) => {
                if (!this.isActive || !this.renderer.insignIoManager)
                    return;
     ...
     */
    findHoveredIoRegion() {
        if (!this.renderer.insignIoManager)
            return null;
    ...
    isHoveringBlock(pos) {
        ...
        const blockMin = new THREE.Vector3(pos[0] + schematicOffset.x - 0.5, pos[1] + schematicOffset.y - 0.5, pos[2] + schematicOffset.z - 0.5);
        const blockMax = new THREE.Vector3(pos[0] + schematicOffset.x + 0.5, pos[1] + schematicOffset.y + 0.5, pos[2] + schematicOffset.z + 0.5);
     ...
     */
    showOverlayForRegion(regionId, mouseX, mouseY) {
        if (!this.renderer.overlayManager || !this.renderer.insignIoManager)
            return;
        const region = this.renderer.insignIoManager.getRegion(regionId);
        if (!region)
            return;
