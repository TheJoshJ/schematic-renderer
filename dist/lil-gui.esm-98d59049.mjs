...
function $(n) {
  let t, i;
  return (t = n.match(/(#|0x)?([a-f0-9]{6})/i)) ? i = t[2] : (t = n.match(/rgb\(\s*(\d*)\s*,\s*(\d*)\s*,\s*(\d*)\s*\)/)) ? i = parseInt(t[1]).toString(16).padStart(2, 0) + parseInt(t[2]).toString(16).padStart(2, 0) + parseInt(t[3]).toString(16).padStart(2, 0) : (t = n.match(/^#?([a-f0-9])([a-f0-9])([a-f0-9])$/i)) && (i = t[1] + t[1] + t[2] + t[2] + t[3] + t[3]), i ? "#" + i : false;
}
const S = {
  isPrimitive: true,
...
class B extends p {
  ...
  _initInput() {
    ...
    let o = false, c, m, u, g, d;
    const f = 5, _ = (s) => {
      c = s.clientX, m = u = s.clientY, o = true, g = this.getValue(), d = 0, window.addEventListener("mousemove", b), window.addEventListener("mouseup", v);
    }, b = (s) => {
      if (o) {
        const h = s.clientX - c, w = s.clientY - m;
        Math.abs(w) > f ? (s.preventDefault(), this.$input.blur(), o = false, this._setDraggingStyle(true, "vertical")) : Math.abs(h) > f && v();
      }
      if (!o) {
        const h = s.clientY - u;
        d -= h * this._step * this._arrowKeyMultiplier(s), g + d > this._max ? d = this._max - g : g + d < this._min && (d = this._min - g), this._snapClampSetValue(g + d);
      }
      u = s.clientY;
    }, v = () => {
      this._setDraggingStyle(false, "vertical"), this._callOnFinishChange(), window.removeEventListener("mousemove", b), window.removeEventListener("mouseup", v);
    }, x = () => {
      this._inputFocused = true;
    }, r = () => {
      this._inputFocused = false, this.updateDisplay(), this._callOnFinishChange();
    };
    this.$input.addEventListener("input", i), this.$input.addEventListener("keydown", l), this.$input.addEventListener("wheel", a, { passive: false }), this.$input.addEventListener("mousedown", _), this.$input.addEventListener("focus", x), this.$input.addEventListener("blur", r);
  }
  _initSlider() {
    ...
    const t = (r, s, h, w, k) => (r - s) / (h - s) * (k - w) + w, i = (r) => {
      ...
      this._setDraggingStyle(true), i(r.clientX), window.addEventListener("mousemove", l), window.addEventListener("mouseup", a);
    }, l = (r) => {
      i(r.clientX);
    }, a = () => {
      this._callOnFinishChange(), this._setDraggingStyle(false), window.removeEventListener("mousemove", l), window.removeEventListener("mouseup", a);
    };
    let o = false, c, m;
    const u = (r) => {
      r.preventDefault(), this._setDraggingStyle(true), i(r.touches[0].clientX), o = false;
    }, g = (r) => {
      r.touches.length > 1 || (this._hasScrollBar ? (c = r.touches[0].clientX, m = r.touches[0].clientY, o = true) : u(r), window.addEventListener("touchmove", d, { passive: false }), window.addEventListener("touchend", f));
    }, d = (r) => {
      if (o) {
        const s = r.touches[0].clientX - c, h = r.touches[0].clientY - m;
        Math.abs(s) > Math.abs(h) ? u(r) : (window.removeEventListener("touchmove", d), window.removeEventListener("touchend", f));
      } else
    ...
    const x = (r) => {
      ...
      this._snapClampSetValue(this.getValue() + h), this.$input.value = this.getValue(), clearTimeout(v), v = setTimeout(_, b);
    };
    this.$slider.addEventListener("mousedown", e), this.$slider.addEventListener("touchstart", g, { passive: false }), this.$slider.addEventListener("wheel", x, { passive: false });
  }
...
class C {
  ...
  constructor({
    ...
    width: l,
    title: a = "Controls",
    closeFolders: o = false,
    injectStyles: c = true,
    touchStyles: m = true
  } = {}) {
    if (this.parent = t, this.root = t ? t.root : this, this.children = [], this.controllers = [], this.folders = [], this._closed = false, this._hidden = false, this.domElement = document.createElement("div"), this.domElement.classList.add("lil-gui"), this.$title = document.createElement("div"), this.$title.classList.add("title"), this.$title.setAttribute("role", "button"), this.$title.setAttribute("aria-expanded", true), this.$title.setAttribute("tabindex", 0), this.$title.addEventListener("click", () => this.openAnimated(this._closed)), this.$title.addEventListener("keydown", (u) => {
      (u.code === "Enter" || u.code === "Space") && (u.preventDefault(), this.$title.click());
    }), this.$title.addEventListener("touchstart", () => {
    }, { passive: true }), this.$children = document.createElement("div"), this.$children.classList.add("children"), this.domElement.appendChild(this.$title), this.domElement.appendChild(this.$children), this.title(a), this.parent) {
      this.parent.children.push(this), this.parent.folders.push(this), this.parent.$children.appendChild(this.domElement);
      return;
    }
    this.domElement.classList.add("root"), m && this.domElement.classList.add("allow-touch-styles"), !E && c && (Y(z), E = true), e ? e.appendChild(this.domElement) : i && (this.domElement.classList.add("autoPlace"), document.body.appendChild(this.domElement)), l && this.domElement.style.setProperty("--width", l + "px"), this._closeFolders = o;
  }
  /**
