...
import { getSharedMemoryPool } from "./workers/SharedMemoryManager";
...
export class WorldMeshBuilder {
    constructor(schematicRenderer, cubane) {
        // @ts-ignore
        ...
        Object.defineProperty(this, "instancedRenderer", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "useInstancedRendering", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        Object.defineProperty(this, "freeWorkers", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "workerQueue", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        ...
        Object.defineProperty(this, "maxWorkers", {
            ...
            configurable: true,
            writable: true,
            value: Math.min(navigator.hardwareConcurrency || 4, 8)
        });
        ...
        Object.defineProperty(this, "chunkSize", {
            ...
            writable: true,
            value: 16
        }); // Default Minecraft chunk size
        // Phase 2 optimizations configuration
        Object.defineProperty(this, "useQuantization", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        // WebGPU Compute
        Object.defineProperty(this, "useGPUCompute", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        });
        Object.defineProperty(this, "computeMeshBuilder", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "gpuInitPromise", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        // WASM Mesh Builder
        Object.defineProperty(this, "useWasmMeshBuilder", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        Object.defineProperty(this, "_timingStats", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "sharedMemoryPool", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "useSharedMemory", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        Object.defineProperty(this, "greedyMeshingEnabled", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        // Batch mode state
        Object.defineProperty(this, "batchPendingChunks", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "batchFinishResolve", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "_batchFinishReject", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        if (type === "chunkBuilt") {
            if (timings) {
                ...
                if (WorldMeshBuilder.stats.chunkCount % 50 === 0) {
                    const avg = (val) => (val / WorldMeshBuilder.stats.chunkCount).toFixed(2);
    ...
    returnWorker(worker) {
        if (!this.workers.includes(worker))
            return; // Worker might have been terminated
    ...
    async computeOcclusionFlags(blockString) {
        try {
    ...
    async precomputePaletteGeometries(palette) {
        ...
        const paletteBlockData = new Array(palette.length);
        const globalMaterialMap = new Map();
        const globalMaterials = [];
        const paletteGeometryData = [];
        // Process all palette entries
        const CONCURRENCY_LIMIT = 8;
        let currentIndex = 0;
        const workerPromises = [];
        const processBlock = async (index) => {
     ...
     */
    async processChunksBatched(allChunks, onProgress) {
            ...
            for (const chunkData of subBatchChunks) {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    if (this.useSharedMemory && this.sharedMemoryPool) {
                        const sharedBuffer = this.sharedMemoryPool.writeChunkInput(chunkId, blocksArray, originX, originY, originZ);
     ...
     */
    createMeshesFromBatchResult(batchResult) {
        const meshes = [];
        for (const meshData of batchResult.meshes) {
            ...
            this.configureMeshForCategory(mesh, meshData.category);
            meshes.push(mesh);
        }
        return meshes;
    }
     ...
     */
    optimizeGeometryGroups(meshData) {
    ...
    async getChunkGeometries(chunkData, renderingBounds) {
        ...
        if (!skipBoundsCheck) {
            if (chunkData.blocks instanceof Int32Array) {
                ...
                blocksToProcess = chunkData.blocks.filter((block) => {
                    const [x, y, z] = block;
                    return (x >= renderingBounds.min.x &&
                        x < renderingBounds.max.x &&
                        y >= renderingBounds.min.y &&
                        y < renderingBounds.max.y &&
                        z >= renderingBounds.min.z &&
        ...
        if (this.useGPUCompute && this.computeMeshBuilder?.isReady) {
            try {
                const gpuResult = await this.computeMeshBuilder.buildChunk(blocksToProcess, [originX, originY, originZ], chunkId);
                if (gpuResult) {
                    return {
        ...
        const workerPromise = new Promise(async (resolve, reject) => {
            ...
            if (this.useSharedMemory && this.sharedMemoryPool && workerBlocks instanceof Int32Array) {
                ...
                const sharedBuffer = this.sharedMemoryPool.writeChunkInput(chunkId, workerBlocks, originX, originY, originZ);
                worker.postMessage({
                    type: "buildChunk",
                    chunkId,
    ...
    async getChunkMesh(chunkData, schematicObject, renderingBounds, preFilteredEntities // Optimization: entities already filtered by WASM
    ) {
        const chunkId = `${chunkData.chunk_x},${chunkData.chunk_y},${chunkData.chunk_z}`;
        if (!this.paletteCache?.isReady) {
            throw new Error("Palette cache not ready. Call precomputePaletteGeometries() first.");
        ...
        if (preFilteredEntities) {
            for (const entity of preFilteredEntities) {
                ...
                if (renderingBounds?.enabled) {
                    if (pos[0] < renderingBounds.min.x ||
            ...
            if (blockEntityMap.size > 0 && blockEntityMap.size < 10000) {
                ...
                let spatialCache = schematicObject._entitySpatialCache;
                if (!spatialCache) {
                    ...
                    schematicObject._entitySpatialCache = spatialCache;
                }
        ...
        if (this.useGPUCompute && this.computeMeshBuilder?.isReady) {
            try {
                const gpuResult = await this.computeMeshBuilder.buildChunk(workerBlocks, [originX, originY, originZ], chunkId);
                if (gpuResult) {
                    buildResult = {
            ...
            if (workerResult.meshes) {
                for (const meshData of workerResult.meshes) {
                    ...
                    if (workerResult.origin) {
                        mesh.position.set(workerResult.origin[0], workerResult.origin[1], workerResult.origin[2]);
                    }
                    this.configureMeshForCategory(mesh, meshData.category);
            ...
            if (tileEntityBlocks.length > 0) {
                ...
                for (const tileBlock of tileEntityBlocks) {
                    ...
                    if (blockString) {
                        try {
                            // const blockString = this.createBlockStringFromPaletteEntry(blockState);
                            const customMesh = await this.cubane.getBlockMesh(blockString, "plains", false, nbtData.nbt || nbtData);
                            if (customMesh) {
    ...
    // Helper methods...
    createBlockStringFromPaletteEntry(blockState) {
        let blockString = blockState.name || "minecraft:stone";
        if (!blockString.includes(":"))
    ...
    extractAllMeshData(rootCubaneObject) {
        const allMeshData = [];
    ...
    createFallbackObject3D(blockString) {
        const mesh = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.7, 0.7), new THREE.MeshBasicMaterial({
            color: 0xee00ee,
            wireframe: true,
    ...
    getPaletteStats() {
        return {
            ...
            memoryEstimate: this.paletteCache?.blockData.reduce((total, blockData) => {
                return (total +
    ...
    enableInstancedRendering(group, merged = false) {
    ...
    async renderSchematicInstanced(schematicObject) {
...
Object.defineProperty(WorldMeshBuilder, "stats", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: {
