...
export class SchematicExporter {
    constructor() {
        ...
        Object.defineProperty(this, "currentExport", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
    ...
    async export(object, options = {}) {
        ...
        try {
            ...
            const exportError = this.createError("UNKNOWN", error instanceof Error ? error.message : "Unknown error", error instanceof Error ? error : undefined);
     ...
     */
    prepareExportGroup(object, options) {
     ...
     */
    async processNormals(group, options) {
        ...
        cloned.traverse((child) => {
            if (child instanceof THREE.Mesh && child.geometry) {
                ...
                if (index) {
                    ...
                    const IndexConstructor = indexArray.constructor;
                    const newIndexArray = new IndexConstructor(indexArray.length);
                    newIndexArray.set(indexArray);
        ...
        if (!indexAttribute) {
            ...
            if (positionAttribute) {
                ...
                for (let i = 0; i < positions.length; i += 9) {
                    // Swap positions
                    this.swapArrayValues(positions, i + 3, i + 6, 3);
                    // Swap normals
                    if (normals) {
                        this.swapArrayValues(normals, i + 3, i + 6, 3);
                    }
                    // Swap UVs (2 components per vertex)
                    if (uvs) {
                        const uvI = (i / 9) * 6;
                        this.swapArrayValues(uvs, uvI + 2, uvI + 4, 2);
                    }
                }
     ...
     */
    swapArrayValues(array, index1, index2, count) {
     ...
     */
    fixMaterialsForExport(group, forceOpaque = false) {
        group.traverse((child) => {
     ...
     */
    setNearestFilterOnMaterial(material) {
        const textureProps = [
            "map",
            "alphaMap",
            "aoMap",
     ...
     */
    async exportGLTF(group, options, filename, startTime) {
        const exporter = new GLTFExporter();
        const isBinary = options.format === "glb";
        const gltfResult = await new Promise((resolve, reject) => {
            exporter.parse(group, (result) => resolve(result), (error) => reject(error), {
                binary: isBinary,
     ...
     */
    async exportOBJ(group, _options, filename, startTime) {
     ...
     */
    async exportSTL(group, _options, filename, startTime) {
     ...
     */
    static getFormatExtension(format) {
        return `.${format}`;
    }
    /**
