...
// State
const paletteGeometries = new Map();
...
function updatePalette(paletteData) {
...
function buildChunk(request) {
    ...
    if (blocks.length > 0) {
        ...
        self.postMessage({ type: "chunkBuilt", chunkId, meshes: [] });
        return;
    }
    ...
    for (const [category, paletteMap] of categoryBatches) {
        ...
        if (positions.length === 0)
            continue;
        const merged = mergeGeometriesWithCulling(geometryData, positions, occlusionFlags, voxelMap, getIndex, originX, originY, originZ);
        if (merged) {
            results.push({
                category,
                ...merged,
            ...
            if (merged.normals)
                transferables.push(merged.normals.buffer);
            if (merged.uvs)
                transferables.push(merged.uvs.buffer);
...
function mergeGeometriesWithCulling(geometries, positions, 
// occlusionFlags: number[], // Unused directly in loop, but kept for signature if needed
_occlusionFlags, voxelMap, getIndex, originX, originY, originZ) {
    let totalVerts = 0;
    let totalIndices = 0;
        ...
        for (let j = 0; j < localIndices.length; j += 3) {
            ...
            if (geom.normals) {
                ...
                if (Math.abs(dx) + Math.abs(dy) + Math.abs(dz) === 1) {
                    ...
                    if (dx === 1)
                        isFlush = Math.abs(v0x - 1.0) < EPSILON || Math.abs(v0x - 0.5) < EPSILON;
                    else if (dx === -1)
