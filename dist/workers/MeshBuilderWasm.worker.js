...
import initWasm, { MeshBuilder } from "../wasm/mesh_builder_wasm.js";
...
const batchAccumulators = new Map();
...
function finishBatch() {
    ...
    for (const [category, acc] of batchAccumulators) {
        ...
        const indices = acc.vertexOffset > 65535 ? new Uint32Array(acc.indices) : new Uint16Array(acc.indices);
        meshes.push({
            category,
            positions,
            normals,
 ...
 */
function buildChunkBatched(request) {
    if (!meshBuilder) {
